...
def substrings_divisible(perm):
    ...
    if perm[3] % 2 != 0 or (perm[2]+perm[3]+perm[4]) % 3 != 0 or perm[5] != 5 and perm[5] != 0:
        return False
    if ((perm[4] * 100 + perm[5] * 10 + perm[6]) % 7 != 0 or
            (perm[5] * 100 + perm[6] * 10 + perm[7]) % 11 != 0):
        return False
    if ((perm[6] * 100 + perm[7] * 10 + perm[8]) % 13 != 0 or
            (perm[7] * 100 + perm[8] * 10 + perm[9]) % 17 != 0):
        return False
    return True
