...
def validate_set(iterable):
    """ check that a given set is a special sum set """
    # get all subsets:
    # https://stackoverflow.com/questions/1482308/how-to-get-all-subsets-of-a-set-powerset
    subsets = chain.from_iterable(combinations(iterable, r)
                                  for r in range(1, len(iterable) + 1))
    subset_sums = set()
    # keep track of max/min sum by set size to check rule 2
    sum_ranges = [[0, 0] for _ in range(len(iterable) + 1)]
    for ss in subsets:
        ss_sum = sum(ss)
...
# brute force: test all possible sets of numbers with sum less than
# sum of set generated by algorithm in question
...
def try_to_find_set(set_min, max_sum, set_len, max_elem):
    """
    use brute force to try to find the optimum set
    """
    if set_len < 1:
        return []
    if set_len == 1:
    ...
    while first_elem < first_element_upper_bound:
        possible_next_numbers = try_to_find_set(
            first_elem+1, max_sum - first_elem, set_len-1, max_elem)
...
def find_set(x):
    """ find the optimal special sum set of size x """
    if x < 1:
        return []
    if x == 1:
        return [1]
    ...
    for f in range(min_elem, first_element_upper_bound):
        ...
        for s in range(f+1, second_element_upper_bound):
            max_el = f + s
            possible_sets = try_to_find_set(
                s+1, new_max_sum - s, x-2, max_el)
