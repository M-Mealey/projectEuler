...
def last_9_pandigital(x):
    """
    check whether the last 9 digits of an integer x are pandigital
...
def first_9_pandigital(x):
    """
    check whether the first 9 digits of a given int are pandigital
    :param x: integer to test
    :return: boolean
    """
    first_9_str = str(x)[:9]
...
def get_first_ten_digits_log(base, exponent):
    """ use a logarithm to get the first 10 digits of a fibonacci number"""
...
def get_fib_number_binet(n):
    ...
    fib_num = round((get_first_ten_digits_log(
        phi, n) - neg_phi**n)/math.sqrt(5))
    return fib_num
